...
const CurrentCampDetailsHeader = () => {
  ...
  if (!userRoles.includes('superadmin') && !user?.currentCampId) {
    return (
      <Alert variant="warning">
        Please select a camp to access features and use the application properly.
  ...
  const { name, location, status, startDate, endDate, vans } = currentCampDetails;
    ...
    <Accordion className=" mb-3">
      <Accordion.Item eventKey="0">
        ...
        <Accordion.Header className="d-flex flex-row justify-content-start align-items-center gap-3">
          ...
          <div className={`text-${status === 'active' ? 'success' : 'danger'} fw-bold`}>{transformText(status)}</div>
        </Accordion.Header>

        {/* Detailed Information in Accordion Body */}
        <Accordion.Body>
          <Row className="">
            <CampInfoItem icon={RiTimeLine} label="Start Date" value={new Date(startDate).toLocaleDateString()} />
            <CampInfoItem icon={RiTimeLine} label="End Date" value={new Date(endDate).toLocaleDateString()} />
            <Col md={4} sm={12}>
              <div className="d-flex align-items-start gap-2">
                <RiTruckLine />
                <h6>Vans Assigned:</h6>
