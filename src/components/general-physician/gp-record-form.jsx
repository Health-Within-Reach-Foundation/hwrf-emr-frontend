...
import { Form, Input, Row, Col, Checkbox, DatePicker, Button, Divider } from 'antd';
import dayjs from 'dayjs';
import patientServices from '../../api/patient-services';
import toast from 'react-hot-toast';
import ComplaintsInput from './gp-complaints';
import EditableMedicineTable from './gp-medicine';
const GPMedicalRecordForm = ({ record, isEdit, onCancel = () => {}, patientData, onSave = () => {}, formFields = [] }) => {
  ...
  const kcoOptions = ['Hypothyroidism', 'Hyperthyroidism', 'Asthama', 'Epilepsy'];

  const findingsOptions = ['Temperature', 'BP', 'Pulse rate', 'Respiratory rate', 'General examination', 'Skin lesion'];
  const systemicExaminationOptions = ['CNS', 'Respiratory', 'Cardio vascular', 'Per abdominal examination'];
  ...
  useEffect(() => {
    if (isEdit && record) {
      form.setFieldsValue({
        ...
        systemicExaminationOptionsDetails: record.systemicExaminationOptionsDetails || {},
      });
  ...
  const selectedSystemicExaminations = Form.useWatch('systemicExamination', form) || [];
      ...
      <Form form={form} layout="vertical" onFinish={onFinish}>
        <Row gutter={[16, 16]}>
          <Col xs={24} sm={12} md={8}>
            ...
            >
              <Input
                type="number"
                onChange={() => calculateBMI(form.getFieldValue('weight'), form.getFieldValue('height'))}
                suffix="kg"
                disabled={!isEditing}
              />
            </Form.Item>
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24}>
            ...
            <Form.Item label={<h6>Complaints</h6>} name="complaints">
              <ComplaintsInput
                formField={formFields?.find((formField) => formField.fieldName === 'complaints')}
                disabled={!isEditing}
              />
            </Form.Item>
            {showOtherComplaints && (
              <Form.Item label={<h6>Other complaints</h6>} name="otherComplaints">
                <Input.TextArea rows={3} disabled={!isEditing} placeholder="If any other complaints please describe it" />
              </Form.Item>
            )}
          </Col>
        </Row>
        ...
        <Row gutter={[16, 16]}>
          {selectedFindings?.includes('Temperature') && (
            <Col xs={24} sm={12} md={8}>
              <Form.Item label={<h6>Temperature details</h6>} name={[`findingsOptionsDetails`, `temperatureDetails`]}>
                <Input placeholder={`Describe temperature details`} disabled={!isEditing} />
              </Form.Item>
            </Col>
          )}
          {selectedFindings?.includes('BP') && (
            <Col xs={24} sm={12} md={8}>
              <Form.Item label={<h6>BP details</h6>} name={[`findingsOptionsDetails`, `bpDetails`]}>
                <Input placeholder={`Describe BP details`} disabled={!isEditing} />
              </Form.Item>
            </Col>
          )}
          {selectedFindings?.includes('Pulse rate') && (
            <Col xs={24} sm={12} md={8}>
              <Form.Item label={<h6>Pulse rate details</h6>} name={[`findingsOptionsDetails`, `pulseRateDetails`]}>
                <Input placeholder={`Describe pulse rate details`} disabled={!isEditing} />
              </Form.Item>
            </Col>
          )}
        </Row>
        <Row gutter={[16, 16]}>
          {selectedFindings?.includes('Respiratory rate') && (
            <Col xs={24} sm={12} md={8}>
              ...
              >
                <Input placeholder={`Describe Respiratory rate details`} disabled={!isEditing} />
              </Form.Item>
            </Col>
          )}
          {selectedFindings?.includes('General examination') && (
            <Col xs={24} sm={12} md={8}>
              ...
              >
                <Input.TextArea rows={3} placeholder={`Describe General examination details`} disabled={!isEditing} />
              </Form.Item>
            </Col>
          )}
          {selectedFindings?.includes('Skin lesion') && (
            <Col xs={24} sm={12} md={8}>
              <Form.Item label={<h6>Skin lesion details</h6>} name={[`findingsOptionsDetails`, `skinLesionDetails`]}>
                <Input.TextArea rows={3} placeholder={`Describe Skin lesion details`} disabled={!isEditing} />
              </Form.Item>
            </Col>
          )}
        </Row>
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24}>
            <Form.Item label={<h6>Systemic Examination</h6>} name="systemicExamination">
              <Checkbox.Group disabled={!isEditing} className="mt-2">
                <Row>
        ...
        <Row gutter={[16, 16]}>
          {selectedSystemicExaminations?.includes('CNS') && (
            <Col xs={24} sm={12} md={8}>
              <Form.Item label={<h6>CNS details</h6>} name={[`systemicExaminationOptionsDetails`, `cnsDetails`]}>
                <Input.TextArea rows={3} placeholder={`Describe CNS details`} disabled={!isEditing} />
              </Form.Item>
            </Col>
          )}
          {selectedSystemicExaminations?.includes('Respiratory') && (
            <Col xs={24} sm={12} md={8}>
              <Form.Item
                ...
                name={[`systemicExaminationOptionsDetails`, `respiratoryDetails`]}
              >
                <Input.TextArea rows={3} placeholder={`Describe respiratory details`} disabled={!isEditing} />
              </Form.Item>
            </Col>
          )}
          {selectedSystemicExaminations?.includes('Cardio vascular') && (
            <Col xs={24} sm={12} md={8}>
              <Form.Item
                ...
                name={[`systemicExaminationOptionsDetails`, `cardioVascularDetails`]}
              >
                <Input.TextArea rows={3} placeholder={`Describe cardio vascular details`} disabled={!isEditing} />
              </Form.Item>
            </Col>
          )}
        </Row>
        <Row gutter={[16, 16]}>
          {selectedSystemicExaminations?.includes('Per abdominal examination') && (
            <Col xs={24} sm={12} md={8}>
              <Form.Item
                label={<h6>Per abdominal examination details</h6>}
                name={[`systemicExaminationOptionsDetails`, `perAbdominalExaminationDetails`]}
              >
                <Input.TextArea rows={3} placeholder={`Describe per abdominal examination details`} disabled={!isEditing} />
              </Form.Item>
            </Col>
          )}
        </Row>
        ...
        <Row gutter={[16, 16]}>
          ...
          >
            <EditableMedicineTable form={form} formField={formFields} disabled={!isEditing} />
          </Form.Item>
        </Row>
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} md={8}>
            <Form.Item label={<h6>Online paid amount</h6>} name="onlineAmount">
              <Input type="number" disabled={!isEditing} onChange={handleAmountChange} />
            </Form.Item>
          </Col>
          <Col xs={24} sm={12} md={8}>
            <Form.Item label={<h6>Offline paid amount</h6>} name="offlineAmount">
              <Input type="number" disabled={!isEditing} onChange={handleAmountChange} />
            </Form.Item>
          </Col>
          <Col xs={24} sm={12} md={8}>
            <Form.Item label={<h6>Total</h6>}>
              <Input type="number" disabled readOnly value={Number(onlineAmount) + Number(offlineAmount)} />
            </Form.Item>
          </Col>
        </Row>
        {isEditing && !isEdit && (
          <Form.Item className="d-flex justify-content-end">
            <Button type="primary" htmlType="submit" className="rounded-0 bg-primary btn-primary" loading={loading}>
              Submit
            </Button>
          </Form.Item>
        )}
