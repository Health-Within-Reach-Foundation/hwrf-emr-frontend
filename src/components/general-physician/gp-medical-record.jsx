...
import { calculateBMI, formatRegisterNumberOfPatient } from '../../utilities/utility-function';
...
const GPMedicalRecord = ({ gpRecords, patientData, onSave = () => {}, formFields = [] }) => {
  ...
  const handlePrint = (record) => {
    const kcoOptions = formFields.find((field) => field.fieldName === 'kco')?.options || [];
    const findingsOptions = formFields.find((field) => field.fieldName === 'findings')?.options || [];
    const systemicExaminationOptions = formFields.find((field) => field.fieldName === 'systemicExamination')?.options || [];
    ...
    const patientParameters = [
      ['Weight:', record.weight, 'Height:', record.height, 'BMI:', calculateBMI(record.weight, record.height)],
    ...
    kcoOptions.forEach((item, index) => {
      ...
      if (record.kco?.includes(item.value)) doc.text('✔', margin + 1, cursorY + index * 5 + 2.5);
    ...
    findingsOptions.forEach((item, index) => {
      ...
      if (record.findings?.includes(item.value)) doc.text('✔', margin + 6, cursorY + 1 + index * 5 + 2.5);
    ...
    systemicExaminationOptions.forEach((item, index) => {
      doc.rect(margin + 5, cursorY + index * 5, 3, 3);
      if (record.systemicExamination?.includes(item.value)) doc.text('✔', margin + 6, cursorY + 1 + index * 5 + 2.5);
      doc.text(item.label, margin + 10, cursorY + index * 5 + 3);
    });
    ...
    doc.autoTable({
      startY: cursorY,
      head: [['Type', 'Medicine', 'Dose', 'When', 'Duration', 'Frequency', 'Notes']],
    ...
    if (cursorY + 20 > 277) addNewPage();
    doc.setFontSize(10);
    doc.text('This is a system-generated report and does not require a signature.', 105, 290, { align: 'center' });
  ...
  const handleDelete = async (recordId) => {
    try {
      ...
      ``;
      if (response) {
        const updatedRecords = gpRecords.filter((record) => record.id !== recordId);
  ...
  return (
    <div>
      ...
      {gpRecords.length === 0 && <h4 className="card-title text-center text-warning">No records found!</h4>}
      ...
      <Timeline
        ...
        items={allGPRecords.map((record, index) => ({
          ...
          children: (
            <Collapse
              className={`${index === 0 ? 'bg-success-subtle' : 'bg-secondary-subtle'} h-auto`}
              items={[
                {
                  key: record.id,
                  label: (
                  ...
                  children: <GPMedicalRecordForm record={record} isEdit={true} onSave={onSave} formFields={formFields} />,
                },
              ]}
            />
          ),
