...
const UserDrawer = ({ open, onClose, userData, allRoles, allSpecialties, onSave }) => {
    ...
    <Drawer
      ...
      footer={
        <div style={{ textAlign: 'right' }}>
          ...
          <Button type="primary" onClick={handleSave} loading={loading} className="btn-primary rounded-0" variant="solid">
            Save
          </Button>
        </div>
      }
    >
      <Form form={form} layout="vertical">
        ...
        <Form.Item name="roles" label="Roles" rules={[{ required: true, message: 'Please select at least one role!' }]}>
          <Select mode="multiple" placeholder="Select roles" options={allRoles} />
        </Form.Item>

        {/* Specialty Selection - Single-select, only visible if doctor is selected */}
        {isDoctorSelected && (
          <Form.Item name="specialty" label="Specialty" rules={[{ required: true, message: 'Please select a specialty!' }]}>
            <Select placeholder="Select specialty" options={allSpecialties} allowClear />
          </Form.Item>
        )}
      </Form>
    </Drawer>
