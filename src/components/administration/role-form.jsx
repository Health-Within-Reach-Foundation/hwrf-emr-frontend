...
const RoleModalForm = ({
  ...
  useEffect(() => {
    if (currentRole) {
      const selectedPermissions = currentRole.permissions?.map((perm) => perm.id) || [];
      form.setFieldsValue({
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (currentRole) {
        // Update role
        response = await rolePermissionService.updateRole(currentRole.id, payload);
      } else {
        // Create new role
  ...
  const sortedPermissions = Object.keys(groupedPermissions).reduce((acc, category) => {
  ...
  return (
    <Modal open={showModal} onCancel={() => setShowModal(false)} centered footer={null} maskClosable={false} width={900}>
                ...
                <div key={category}>
                  ...
                  <Row gutter={[16, 16]} className="mb-2">
                    {sortedPermissions[category].map((perm) => (
                      <Col xs={24} sm={12} md={8} lg={6} key={perm.id} style={{ display: 'flex', alignItems: 'center' }}>
                        <Checkbox value={perm.id} style={{ marginRight: 8 }} className="w-auto">
