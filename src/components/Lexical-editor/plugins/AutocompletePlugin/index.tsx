...
function $search(selection: null | BaseSelection): [boolean, string] {
  ...
  const text = node.getTextContent();
  let word = '';
  for (let i = node.getTextContentSize() - 1; i >= 0 && text[i] !== ' '; i--) {
    word = text[i] + word;
  }

  return word.length > 0 ? [true, word] : [false, ''];
}
...
export default function AutocompletePlugin(): JSX.Element | null {
  ...
  useEffect(() => {
    ...
    const $clearSuggestion = () => {
      const autocompleteNode = autocompleteNodeKey ? $getNodeByKey(autocompleteNodeKey) : null;
      if (autocompleteNode && autocompleteNode.isAttached()) {
        autocompleteNode.remove();
      }
      if (searchPromise) {
        searchPromise.dismiss();
  ...
  query = (searchText: string): SearchPromise => {
    ...
    const promise: Promise<null | string> = new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        const match = this.DATABASE.find((word) =>
        ...
        resolve(autocompleteChunk || null);
      }, this.LATENCY);
    });
