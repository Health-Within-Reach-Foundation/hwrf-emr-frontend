...
const ChildTeethSelector = ({ selectedTeeth = [], onChange, isEdit = false }) => {
  ...
  const handleSelect = (id) => {
    ...
    if (isEdit) {
      ...
      updatedTeeth = teethState.includes(id) ? teethState.filter((tooth) => tooth !== id) : [...teethState, id];
    }
      ...
      <div className="child-teeth-image">
        {teethPositions.map((tooth) =>
          tooth.id > 70 ? (
            <button
              key={tooth.id}
              className={`child-tooth-button ${teethState?.includes(tooth.id) ? 'child-selected' : ''}`}
              style={{
                bottom: tooth.top,
                right: tooth.left,
