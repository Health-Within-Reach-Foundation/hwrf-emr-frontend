...
const updateCampById = async (campId, campData) => {
  try {
    const response = await apiClient.patch(`/clinics/camps/${campId}`, campData);
    return response.data;
  } catch (error) {
    if (error.response) {
      console.error('Error response:', error.response.data);
...
const getCampById = async (campId) => {
  try {
    ...
    if (error.response) {
      console.error('Error response:', error.response.data);
      throw new Error(error.response.data.message || 'Failed to fetch camp details');
    } else {
      console.error('Unexpected error:', error.message);
      throw new Error('An unexpected error occurred while fetching camp details');
    }
  }
};
...
export const getCampsAnalytics = async (startDate, endDate) => {
  try {
    const response = await apiClient.post('/clinics/camps/analytics', { endDate, startDate });
    console.log('responsesaaa', response);
    return response.data;
  } catch (error) {
    if (error.response) {
      console.error('Error response:', error.response.data);
      throw new Error(error.response.data.message || 'Failed to fetch camps analytics');
    } else {
      console.error('Unexpected error:', error.message);
      throw new Error('An unexpected error occurred while fetching camps analytics');
    }
  }
};
