...
// ─────────────────────────────────────────────────────────────────────────────
// BUG FIX 3: getPatientForExport (email export)
//   BEFORE: accepted only maxRecords, sent no filters → backend emailed ALL patients.
//   AFTER:  accepts filters object and passes them as query params so only
//           filtered patients are emailed.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// BUG FIX 2: exportPatientsDownload (file download)
//   BEFORE: this function did not exist in patient-services.js, so the
//           "Export Visible" button was only exporting the current page (50 rows).
//   AFTER:  calls /export-download with filters and returns a Blob so
//           the frontend can trigger a real file download of ALL filtered patients.
// ─────────────────────────────────────────────────────────────────────────────
const exportPatientsDownload = async ({ name = "", address = "", service = "" } = {}) => {
  try {
    const response = await apiClient.get("/patients/export-download", {
      params: { name, address, service },
      responseType: "blob",   // ← important: tells axios to return a Blob
      timeout: 60000,
    });
    return response.data; // Blob – caller uses saveAs() on this
  } catch (error) {
    if (error.response) {
      console.error("Error response:", error.response.data);
      ...
      console.error("Unexpected error:", error.message);
      throw new Error("An unexpected error occurred while downloading export");
    }
  }
};
...
const updatePatientDiagnosis = async (diagnosisId, patientDiagnosisData) => {
  try {
    ...
    if (error.response) {
      ...
      console.error("Unexpected error:", error.message);
      throw new Error("An unexpected error occurred while updating diagnosis");
    }
  }
};
...
const addTreatmentByDiagnosis = async (treatmentData) => {
  try {
    ...
    if (error.response) {
      ...
      console.error("Unexpected error:", error.message);
      throw new Error("An unexpected error occurred while adding treatment");
    }
  }
};
...
export default {
  ...
  searchPatients,
  getPatientForExport,
  exportPatientsDownload,       // ✅ NEW: added for download export
